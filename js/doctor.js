...
(function(){
	...
	main.getElements('.lang-ace').each(function(el){
		...
		var wrap = new Element('div.alert').adopt(
...
//				body.scrollTo(0, main.getElement(target).getPosition().y - 40);
	...
	var buildWindow = function(useWindow, el){
		...
		if (!useWindow) {
			...
			pop.openWin('javascript:""');
			pop.addEvent('close', function(){
				this.removeEvents('close');
				toggleState(el);
			});

			return pop.window.document;
		}
	};
	...
	var handleClick = function(e, el){
		...
		toggleState(el);
		var body = buildWindow(el.get('data-window'), el);
